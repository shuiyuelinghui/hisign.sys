...
export default class PreacceptApprovalController {
    ...
    listenConfirmEvent($event) {
        this.$scope.$on('approveSubmitSuccess', ()=> {
            this.admindesignsService.openMagnificPopup( './templates/modal/confirmPopup.html',this, {}, this.fetchOpinionData(),this.closePopup)
        });
    }
    ...
    bulidSubmitParams() {
        ...
        this.submitParams.result = this.configData.resultValue == '1' ? '0402' : '0401';
        //判断是否重新预受理审核
        if (this.flag == '1') {
            this.submitParams.operateType = '2';
        }
    }
    //提交数据
    submitData() {
    ...
    goBack() {
        this.goState('app.task.todo.personal',{pageId:'ap1030'});
    }
    //关闭弹窗
    closePopup() {
...
PreacceptApprovalController.$inject = [
    '$scope',
    '$timeout',
    '$stateParams',
    'UtilityService',
